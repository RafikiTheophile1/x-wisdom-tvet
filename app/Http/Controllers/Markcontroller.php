...
public function viewReport(Request $request)
{
    ...
    $select = Marks::with(['trainee', 'trade'])
        ->when($query, function ($q) use ($query) {
            $q->whereHas('trainee', function ($q2) use ($query) {
                $q2->where('Traineefname', 'like', "%$query%")
                   ->orWhere('Traineelname', 'like', "%$query%");
            })
            ->orWhereHas('trade', function ($q3) use ($query) {
                $q3->where('Trade_name', 'like', "%$query%");
            });
        })
        ->get();
